...
<div class="bg-white shadow overflow-hidden sm:rounded-lg">
    <div class="px-4 py-5 sm:p-6">
        <form id="edit-staff-form" action="/staff/<?= $staff['id'] ?>" method="POST" class="space-y-6">
            <input type="hidden" name="_method" value="PUT">
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    <label for="employee_id" class="block text-sm font-medium text-gray-700">Employee ID</label>
                    <input type="text" name="employee_id" id="employee_id" value="<?= htmlspecialchars($staff['employee_id'] ?? '') ?>" required
                    ...
                    <div class="mt-2 space-y-2">
                        <div class="flex items-center">
                            <input id="teaching" name="position" type="radio" value="Teacher" 
                                <?php if (isset($staff['position']) && $staff['position'] === 'Teacher'): ?>checked<?php endif; ?>
                        ...
                        <div class="flex items-center">
                            <input id="non-teaching" name="position" type="radio" value="Non-Teaching Staff" 
                                <?php if (isset($staff['position']) && $staff['position'] === 'Non-Teaching Staff'): ?>checked<?php endif; ?>
                                class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500 position-radio">
                ...
                <div>
                    ...
                    <input type="text" name="first_name" id="first_name" value="<?= htmlspecialchars($staff['first_name'] ?? '') ?>" required
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>

                <div>
                    <label for="last_name" class="block text-sm font-medium text-gray-700">Last Name</label>
                    <input type="text" name="last_name" id="last_name" value="<?= htmlspecialchars($staff['last_name'] ?? '') ?>" required
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>

                <div>
                    <label for="department" class="block text-sm font-medium text-gray-700">Department</label>
                    <input type="text" name="department" id="department" value="<?= htmlspecialchars($staff['department'] ?? '') ?>"
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="phone" id="phone" value="<?= htmlspecialchars($staff['phone'] ?? '') ?>"
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>

                <div>
                    <label for="hire_date" class="block text-sm font-medium text-gray-700">Hire Date</label>
                    <input type="date" name="hire_date" id="hire_date" value="<?= htmlspecialchars($staff['hire_date'] ?? '') ?>"
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>

                <div>
                    <label for="salary" class="block text-sm font-medium text-gray-700">Salary</label>
                    <input type="number" name="salary" id="salary" step="0.01" min="0" value="<?= htmlspecialchars($staff['salary'] ?? '') ?>"
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status"
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        <option value="active" <?= (isset($staff['status']) && $staff['status'] === 'active') ? 'selected' : '' ?>>Active</option>
                        <option value="inactive" <?= (isset($staff['status']) && $staff['status'] === 'inactive') ? 'selected' : '' ?>>Inactive</option>
                    </select>
                </div>

                <!-- Subjects section for teaching staff -->
                <div id="subjects-field" class="sm:col-span-2 <?php echo (isset($staff['position']) && $staff['position'] === 'Teacher') ? '' : 'hidden'; ?>">
...
if (typeof initializeEditForm === 'undefined' || initializeEditForm === true) {
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize the position radios
