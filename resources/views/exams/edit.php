...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center mb-6">
            ...
            <div>
                <a href="#" onclick="closeEditModal(); return false;" class="bg-gray-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-700">
                    Cancel
                </a>
            </div>
        </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                <form action="/exams/<?= $exam['id'] ?>" method="POST" class="space-y-6" id="examForm">
                    <input type="hidden" name="_method" value="PUT">
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-700">Exam Name</label>
                            <input type="text" name="name" id="name" value="<?= htmlspecialchars($exam['name'] ?? '') ?>" required
                        ...
                        <div>
                            ...
                            <select name="academic_year_id" id="academic_year_id" required
                                ...
                                <?php if (isset($currentAcademicYear)): ?>
                                    <option value="<?= $currentAcademicYear['id'] ?>" <?= (isset($exam['academic_year_id']) && $exam['academic_year_id'] == $currentAcademicYear['id']) ? 'selected' : '' ?>>
                        ...
                        <div>
                            ...
                            <select name="term" id="term" required
                                ...
                                <?php if (isset($currentAcademicYear) && !empty($currentAcademicYear['term'])): ?>
                                    <option value="<?= htmlspecialchars($currentAcademicYear['term']) ?>" <?= (!isset($exam['term']) || $exam['term'] == $currentAcademicYear['term']) ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($currentAcademicYear['term']) ?>
                                    </option>
                                <?php endif; ?>
                                <?php if (!isset($currentAcademicYear) || empty($currentAcademicYear['term']) || $currentAcademicYear['term'] != '1st Term'): ?>
                                    <option value="1st Term" <?= (isset($exam['term']) && $exam['term'] == '1st Term') ? 'selected' : '' ?>>1st Term</option>
                                <?php endif; ?>
                                <?php if (!isset($currentAcademicYear) || empty($currentAcademicYear['term']) || $currentAcademicYear['term'] != '2nd Term'): ?>
                                    <option value="2nd Term" <?= (isset($exam['term']) && $exam['term'] == '2nd Term') ? 'selected' : '' ?>>2nd Term</option>
                                <?php endif; ?>
                                <?php if (!isset($currentAcademicYear) || empty($currentAcademicYear['term']) || $currentAcademicYear['term'] != '3rd Term'): ?>
                                    <option value="3rd Term" <?= (isset($exam['term']) && $exam['term'] == '3rd Term') ? 'selected' : '' ?>>3rd Term</option>
                                <?php endif; ?>
                                    ...
                                    <?php foreach ($classes as $class): ?>
                                        <div class="flex items-center mb-2">
                                            <input type="checkbox" name="class_ids[]" value="<?= $class['id'] ?>" id="class_<?= $class['id'] ?>"
                                                ...
                                                <?= (isset($assignedClassIds) && in_array($class['id'], $assignedClassIds)) ? 'checked' : '' ?>>
                        ...
                        <div>
                            ...
                            <input type="date" name="date" id="date" value="<?= htmlspecialchars($exam['date'] ?? '') ?>" required
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="3"
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"><?= htmlspecialchars($exam['description'] ?? '') ?></textarea>
                        </div>
                            ...
                            <select name="grading_scale_id" id="grading_scale_id" required
                                ...
                                <?php if (isset($gradingScales) && !empty($gradingScales)): ?>
                                    <?php foreach ($gradingScales as $scale): ?>
                                        <option value="<?= $scale['id'] ?>" <?= (isset($exam['grading_scale_id']) && $exam['grading_scale_id'] == $scale['id']) ? 'selected' : '' ?>>
                        ...
                        <div class="sm:col-span-2">
                            <div class="flex items-center">
                                <input type="checkbox" name="has_classwork" id="has_classwork" value="1"
                                    class="h-4 w-4 text-indigo-600 border-gray-300 rounded" <?= (isset($exam['has_classwork']) && $exam['has_classwork']) ? 'checked' : '' ?>>
                            ...
                            <div id="classwork-fields" class="mt-4 <?= (isset($exam['has_classwork']) && $exam['has_classwork']) ? '' : 'hidden' ?>">
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    <input type="number" name="classwork_percentage" id="classwork_percentage" min="0" max="100" step="0.1"
                                        ...
                                        placeholder="0.00" value="<?= isset($exam['classwork_percentage']) ? htmlspecialchars($exam['classwork_percentage']) : '' ?>">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (academicYearSelect && currentAcademicYearId) {
        academicYearSelect.addEventListener('change', function() {
            if (this.value !== currentAcademicYearId) {
                const confirmChange = confirm(`You are about to change from the current academic year to a different one. This may affect data consistency. Are you sure you want to proceed?`);
                if (!confirmChange) {
    ...
    if (hasClassworkCheckbox && classworkFields) {
        hasClassworkCheckbox.addEventListener('change', function() {
            if (this.checked) {
                classworkFields.classList.remove('hidden');
            } else {
                classworkFields.classList.add('hidden');
            }
        });
    }
});
